...
>({
  ...
  Component: (
    ...
    const refPrevIsEditMode = useRef(stateIsEditMode);

    useEffect(() => {
      if (refPrevIsEditMode.current !== isEditMode) {
        onEditModeChange?.({
          isEditModeNew: isEditMode,
          isEditModeOld: refPrevIsEditMode.current,
        });
        refPrevIsEditMode.current = isEditMode;
      }
    }, [isEditMode, onEditModeChange]);

    useEffect(() => {
      if (stateIsEditMode !== isEditMode) {
        setStateIsEditMode({ newStateOrSetStateAction: isEditMode });
      }
    }, [isEditMode, stateIsEditMode, setStateIsEditMode]);
